...
def filter_claims_for_claimant(claims: List[Claim], friend: Friend) -> Union[Claim, None]:
...
def get_items_claimed_for_friend(receipt: Receipt, friend: Friend) -> List[LineEntry]:
    return list(filter(
        lambda item: (item.claims is not None) and any(claim.claimant == friend for claim in item.claims),
...
def get_total_cost_for_friend(items: List[LineEntry], friend: Friend) -> float:
    total_cost = sum(
        item.item_cost * filter_claims_for_claimant(item.claims, friend).portion / item.total_claimed_portions
        for item in items
    )
    return total_cost
...
def summarize(receipt: Receipt, friends: List[Friend]):
    ...
    if add_tip == "Even split":
        receipt.line_entries.append(
            LineEntry(item_name="Tip", item_cost=total_tip, claims=[Claim(claimant=f) for f in friends])
        )

    if add_tip == "Custom split":
        st.markdown("_Quick Tip: The fields below are pre-filled with the amount all participants paid -  this means each diner pays a tip precisely on what they bought._")
        ...
        for i, friend in enumerate(friends):
            ...
            with tip_portion_cols[i]:
                tip_portion = st.text_input(label=f"tip_prop_{friend.name}", value=total_cost, label_visibility="collapsed")
                tip_line_entry.claims.append(Claim(claimant=friend, portion=float(tip_portion)))
